document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (isMeeshoTab) {
    ...
    activeBadge.textContent = "Not on Meesho";
  ...
  searchInput.addEventListener("input", () => {
    if (searchInput.value.length > 0) clearBtn.removeAttribute("hidden");
    else clearBtn.setAttribute("hidden", "true");
  });
  ...
  searchForm.addEventListener("submit", async (e) => {
    ...
    await chrome.storage.local.set({
      ...
      extractionState: {
        isExtracting: true,
        progressMessage: "Searching & starting extraction...",
        scannedCount: 0, totalMatching: 0, products: [],
        tabId: currentTab?.id, error: null
      }
    });
  ...
  openMeeshoBtn.addEventListener("click", async () => {
    if (currentTab?.id && !currentTab.url?.startsWith("chrome://"))
      await chrome.tabs.update(currentTab.id, { url: "https://www.meesho.com" });
  ...
  exportExcelBtn.addEventListener("click", () => {
    ...
    try {
      const rows = currentProducts.map(p => {
        ...
        const codeVal    = isolatedCode || p.code || "N/A";
        // Full Lens output text — everything the Lens page returned
        const lensText   = p.lens_text || "N/A";
        return {
          "Product Name":       p.product_name  || "N/A",
          "Price":              p.price          || "N/A",
          "Rating / 5":         p.rating !== null ? p.rating : "N/A",
          "Code":               codeVal,
          "Lens Output":        lensText,
      ...
      // Make Product Link and Image URL hyperlinks
      const range = XLSX.utils.decode_range(ws["!ref"]);
      for (let R = range.s.r + 1; R <= range.e.r; ++R) {
        // Product Link is col 5 (F)
        const linkCell = XLSX.utils.encode_cell({ r: R, c: 5 });
        if (ws[linkCell]?.v) ws[linkCell].l = { Target: String(ws[linkCell].v) };
        // Image URL is col 6 (G)
        const imgCell = XLSX.utils.encode_cell({ r: R, c: 6 });
        if (ws[imgCell]?.v) ws[imgCell].l = { Target: String(ws[imgCell].v) };
      }
      ...
      ws["!cols"] = [
        ...
        { wch: 10 },  // Rating
        { wch: 18 },  // Code
        { wch: 60 },  // Lens Output (wide — full text)
        { wch: 48 },  // Product Link
        { wch: 55 },  // Image URL
      ];
      ...
      XLSX.writeFile(
        wb,
        `meesho_${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}_${pad(d.getHours())}-${pad(d.getMinutes())}.xlsx`
      );
  ...
  function updateUIState(state) {
    ...
    statusDot.className = isExtracting
      ? "status-dot active"
      : (msg === "Completed" ? "status-dot completed" : "status-dot");
    ...
    exportStatusText.textContent = hasProducts
      ? `${currentProducts.length} products ready`
      : "0 products ready";
  ...
  function renderResultsTable(products) {
        ...
        <tr class="empty-row">
          <td colspan="6">
            <div class="empty-state">
              ...
              <span>No results yet. Search a product and start extraction.</span>
            </div>
          </td>
        </tr>`;
      return;
    ...
    resultsTableBody.innerHTML = products.map(p => {
      ...
      const imgHtml = p.image_url
        ? `<img class="thumb-img" src="${esc(p.image_url)}" alt="product" loading="lazy"
             onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">`
        + `<div class="thumb-placeholder" style="display:none">
             <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" opacity="0.4">
      ...
      const lensTitle    = esc(p.lens_text || "");

      let codeHtml = `<span class="null-code">—</span>`;
      if (isolatedCode) {
        codeHtml = `<span class="code-pill" title="${esc(isolatedCode)}">${esc(isolatedCode)}</span>`;
      } else if (displayText) {
        codeHtml = `<span class="code-pill raw-text" title="${lensTitle}">${esc(displayText)}</span>`;
      }

      return `<tr>
        <td class="col-thumb">${imgHtml}</td>
        <td class="prod-name-cell" title="${name}">${name}</td>
        <td class="prod-price-cell">${price}</td>
        <td class="prod-rating-cell"><span class="rating-pill">${rating} ★</span></td>
        <td>${codeHtml}</td>
        <td style="text-align:center">
          <a href="${esc(p.product_link)}" target="_blank" class="prod-link-btn" title="Open on Meesho">
